...
class SVDDevicesHelper:
    ...
    def get_peripheral(self, name):
        return next(
            (p for d in self.__devices for p in d.peripherals if p.name == name),
            None,
        )
    ...
    def get_format(r, fmt):
        ...
        if fmt is None:
            if pointer_size == register_size and (
                len(r.fields) == 0
                or (len(r.fields) == 1 and r.fields[0].bit_width == register_size)
            ):
    ...
    def get_addr_and_value(p, r, fmt, styling=False):
        pointer_size = 8 * gdb.lookup_type("long").pointer().sizeof
        register_size = r.size if r.size is not None and r.size != 0 else pointer_size
        gdb_pointer = (
            gdb.selected_frame()
            .architecture()
        ...
        try:
            if fmt == "address":
                value = (
                    addr.dereference().cast(gdb_pointer).format_string(styling=styling)
                )
            else:
...
class SVDInfo(SVDCommon):
    ...
    def invoke(self, argument, from_tty):
        ...
        if len(args) == 0:
            ...
            info_iterator = self._svd_devices_helper.info_register(args[0], args[1])
        else:
...
class SVDSearch(SVDCommon):
    ...
    def invoke(self, argument, from_tty):
        ...
        for (
            ...
            rname,
            addr,
        ) in self._svd_devices_helper.search_by_region(start, end):
...
class SVD(SVDDevicesHelper, Dashboard.Module):  # noqa: F821
    ...
    def get_register(self, peripheral, register, fmt=None):
        ...
        if r is None:
            raise Exception(
                f"Register {register} not found for peripheral {peripheral}"
            )
        ...
        return (
            p,
            r,
            SVDDevicesHelper.get_format(r, fmt),
            None,
        ), False

    def add(self, arg):
        try:
            args, fmt = SVDDevicesHelper.split_argv(gdb.string_to_argv(arg))
            peripheral, register = args
        except Exception:
